...
source /home/salnabulsi/.thesis-py38/bin/activate && python train_baseline.py --dataset ${dataset} --batch-size ${batch_size} --epochs ${epochs} --resplit ${resplit} --local ${local} --resized-images ${resized} --checkpoint ${checkpoint} --run-folder ${run_folder} --accuracy ${accuracy} --seed ${10} --real-data ${11}  --caption-file ${12} --baseline-model ${13} --height ${14}
